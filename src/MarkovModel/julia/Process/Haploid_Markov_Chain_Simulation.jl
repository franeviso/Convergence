using Distributions
using LinearAlgebra
using Statistics # Needed for mean/variance if analyzing simulation output
...
const N = 100000       # Population size (small N means high drift)
const L = 10        # Sequence length (4 genotypes: 00, 01, 10, 11)
const MU = 0.01    # Mutation rate per locus
...
# Note: The function build_transition_matrix (for the full T matrix) is retained 
# for completeness but is commented out here as it is computationally intensive 
# and not required for the simulation itself.
