...
i;
    ...
    nonsister = sort(G(i,1:24),'descend');
    nonsisterN = find(G(i,1:24) >= nonsister(1,12));  
    nonsister = setdiff(vector,nonsisterN);
    ...
    for j = 1:length(nonsister);
        ...
        %P(i,nonsister(1,j));
        %mean(P(i,sister));
        if P(i,nonsister(1,j)) > mean(P(i,nonsisterN));
